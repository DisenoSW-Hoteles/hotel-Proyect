...
import { AddChargeRequestDto } from '../../data/models';
...
export class CheckOutComponent implements OnInit, OnDestroy {
    ...
    this.reservationService
      ...
      .pipe(
        switchMap((reservation) =>
          this.folioService.loadFolioByReservation(reservation.id)
        ),
  ...
  onAddCharge(): void {
    ...
    const folioId = this.folioService.getCurrentFolio()?.id;
    if (!folioId) return;
  ...
  onProceedToPayment(): void {
    const folio = this.folioService.getCurrentFolio();
    if (!folio) return;
